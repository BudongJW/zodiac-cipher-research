...
│   └── external-leads.md     外部线索汇编：Z13 原件、其他密文、crib、2025–2026 年外部项目
├── data/
│   ├── README.md             转录约定、来源与逐符号说明
│   ├── z408.txt z340.txt     已解密文转录
│   ├── z13.txt  z32.txt      未解密文转录
│   ├── claims.json           历次 Z13 / Z32 破译声明的结构化记录（M3 评估用，30 条）
│   ├── other/                其他被归于 Zodiac 的密文（Z148、Albany、Z263，对照数据）
│   ├── glyphs/               由公有领域扫描件生成的字形对比图
│   └── solutions/            Z408 / Z340 已知解（用于回归测试）
├── zkc/                      Python 分析工具包（统计、换位、密钥、n-gram、求解器、合成密文、姓名匹配、crib 分析）
...
| M3 | ✅ | 为 30 条历次 Z13 / Z32 声明（含 2025–2026 年新声明）统一打分：**无一达到 A 级**（B 10、C 6、D 14）；Z32 各声明与 Z340 / Z408 密钥的一致均在偶然水平（[m3-report](docs/m3-report.md)） |
